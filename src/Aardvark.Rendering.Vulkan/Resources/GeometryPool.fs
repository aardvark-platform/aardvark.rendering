                        ...
                        device.GraphicsFamily.RunSynchronously(
                            { new Command() with
                                ...
                                member x.Enqueue(cmd) =
                                    ...
                                    // try/finally keeps copyInfo's stack slot alive across the native call:
                                    // a call inside `try` is never in tail position, so the F# compiler can't
                                    // emit the erroneous .tail that drops the frame (dotnet/fsharp#18689).
                                    // Zero-alloc, unlike NativePtr.pin which wraps the value in a pinned array.
                                    try VkRaw.vkCmdCopyBuffer(cmd.Handle, scratchBuffer.Handle, handle, 1u, &&copyInfo)
                                    finally ()
                            }
                        )
            )
