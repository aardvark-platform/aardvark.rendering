...
module Test = 
    ...
    type Linked<'a>(value : 'a) =
        let mutable prev : Option<Linked<'a>> = None
        let mutable next : Option<Linked<'a>> = None
        ...
        interface ILinked<Linked<'a>> with
            member x.Prev
                with get() = prev
                and set p = prev <- p
    ...
    let run() = 
        let bla = Trie<Linked<int>>()

        bla.Add([1;2;3], Linked 6)
        bla.Add([1;2;4], Linked 7)
        bla.Add([0;1], Linked 1)
        bla.Add([0;1;4], Linked 5)
