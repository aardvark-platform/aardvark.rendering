...
module HelloWorld =
    ...
    let testLoD() =
        ...
        let view = CameraView.LookAt(V3d(150.0,150.0,150.0), V3d(50,50,50), V3d.OOI)
        let perspective = 
            win.Sizes 
        ...
        let rand = Random()
        let randomPoints (bounds : Box3d) pointCount =
                ...
                for x in 0 .. 10 do
                    for y in 0 .. 10 do
                        for z in 0 .. 3 do
                            ...
                            yield { bounds = bounds; levels = 3; createLevel = fun level -> randomPoints bounds ((1 <<< level)*10) }
            }
