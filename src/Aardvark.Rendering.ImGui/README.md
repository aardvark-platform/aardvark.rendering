...
## Integration
1. Reference `Aardvark.ImGui` in your project.
...
1. Define your GUI by setting the render function of ImGui:
    ...
    ```
    This function is called every frame to render the GUI. The methods from the `ImGui` class contain all the relevant widgets and controls. For convenience, Aardvark.ImGui provides variants for `cval` and other Aardvark-specific types. As a reference, take a look at the demo window and [its source code](https://github.com/ocornut/imgui/blob/master/imgui_demo.cpp).
1. Add the GUI to your scene graph:
    ```fsharp
    let cmd = RenderCommand.Ordered [scene; gui]
    window.Scene <- Sg.execute cmd
    ```
    Note that `Aardvark.ImGui.Instance` implements `ISg` and can be added to the scene graph directly. Make sure that the GUI is rendered after the rest of the scene graph by using render commands.