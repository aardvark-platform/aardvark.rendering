...
module ``SceneGraph Tests`` =
    ...
    [<Test>]
    let ``[Sg] Check Completeness``() =
        IntrospectionProperties.CustomEntryAssembly <- AppDomain.CurrentDomain.Load("Aardvark.SceneGraph")
        ...
        for (att, expected) in semantics do
            for t in sgTypes do
                if t.DeclaringType = sgModule then
                    ...
                    Assert.IsTrue(hasRule, sprintf "no semantic %A for type %s" att (prettyName t))
