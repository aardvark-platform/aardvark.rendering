...
////        VK_STRUCTURE_TYPE_WIN32_SURFACE_CREATE_INFO_KHR = 1000009000,
    ...
    let knownStructs =
        Map.ofList [
            "VkClusterAccelerationStructureGeometryIndexAndGeometryFlagsNV", fun () -> ()
            "VkClusterAccelerationStructureBuildTriangleClusterInfoNV", fun () -> ()
            "VkClusterAccelerationStructureBuildTriangleClusterTemplateInfoNV", fun () -> ()
        ]
        ...
        for s in structs do
            ...
            | Some alias ->
                if s.name <> alias then
                    ...
                    printfn ""
            | None ->
                match Map.tryFind s.name knownStructs with
                | Some f ->
                    printfn "type %s = struct end" s.name
