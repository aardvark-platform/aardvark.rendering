...
module private OpenGL =
    ...
    let createSwapchain (runtime : Runtime) (signature : IFramebufferSignature) (ctx : Aardvark.Rendering.GL.ContextHandle) (glfw : Glfw) (win : nativeptr<WindowHandle>) (size : V2i) =
        ...
        let colorSig = signature.ColorAttachments |> Map.toSeq |> Seq.head |> snd |> snd 
        let defaultFramebuffer = 
            new Framebuffer(
                runtime.Context, signature, 
                (fun _ -> 0), 
                ignore, 
                [0, DefaultSemantic.Colors, Aardvark.Rendering.GL.Renderbuffer(runtime.Context, 0, size, RenderbufferFormat.Rgba8, colorSig.samples, 0L) :> IFramebufferOutput], None
            ) 
    ...
    let createSurface (runtime : Runtime) (cfg: WindowConfig) (glfw : Glfw) (win : nativeptr<WindowHandle>) =
        ...
        let signature =
            runtime.CreateFramebufferSignature(cfg.samples, [
                DefaultSemantic.Colors, RenderbufferFormat.Rgba8
                DefaultSemantic.Depth, RenderbufferFormat.Depth24Stencil8
            ])
        let handle = Aardvark.Rendering.GL.ContextHandle(ctx, info)
