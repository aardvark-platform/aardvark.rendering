...
module TraceObject =
    ...
    let customIndex (index : aval<uint32>) (obj : TraceObject) =
        { obj with CustomIndex = index }

    let customIndex' (index : uint32) (obj : TraceObject) =
...
module TraceObjectBuilder =
    ...
    type TraceObjectBuilder() =
        ...
        member x.CustomIndex(o : TraceObject, i : aval<uint32>) =
            o |> TraceObject.customIndex i

        member x.CustomIndex(o : TraceObject, i : uint32) =
            o |> TraceObject.customIndex' i

        member x.CustomIndex(o : TraceObject, i : aval<int32>) =
            o |> TraceObject.customIndex (i |> AVal.mapNonAdaptive uint32)

        member x.CustomIndex(o : TraceObject, i : int32) =
            o |> TraceObject.customIndex' (uint32 i)
