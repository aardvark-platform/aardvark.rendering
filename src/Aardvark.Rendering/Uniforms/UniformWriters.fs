...
module GLSLType =
    ...
    let rec toString = function
        ...
        | Int (signed, width) ->
            let prefix = if signed then "" else "u"
            $"{prefix}int{width}"

        | Float width ->
            $"float{width}"

        | Vec (dim, Int (false, (8 | 16 as width))) ->
            let suffix = if width = 8 then "b" else "us"
            $"C{dim}{suffix}"

        | Vec (dim, Int (signed, 32)) ->
            let suffix = if signed then "i" else "ui"
            $"V{dim}{suffix}"

        | Vec (dim, Float (32 | 64 as width)) ->
            let suffix = if width = 32 then "f" else "d"
            $"V{dim}{suffix}"

        | Mat (cols, rows, Float 32) ->
            $"M{rows}{cols}f"
        ...
        | Struct (name, fields, _) ->
            let fields = fields |> List.map (fun (n, t, _) -> $"{n} : {toString t}" ) |> String.concat "; "
            $"{name} {{ {fields} }}"
    ...
    let rec sizeof (t : GLSLType) =
        ...
        | Int (_, width)        -> width / 8
        | Float width           -> width / 8
        | Vec (dim, elem)       -> dim * sizeof elem
        | Mat (cols, _, elem)   -> 4 * cols * sizeof elem // Matrix rows need to be padded to 4 elements according to std140
        | Array(len, _, stride) -> len * stride
        | Struct (_, _, size)   -> size
        | _ -> raise <| ArgumentException($"Cannot determine size of {toString t}")
...
module UniformWriters =
    ...
    module private List =
        let rec mapOptionV (f : 'a -> 'b voption) (l : list<'a>) =
            match l with
            | [] -> ValueSome []
            | h :: rest ->
                match f h, mapOptionV f rest with
                | ValueSome h, ValueSome t -> ValueSome (h :: t)
                | _ ->  ValueNone

    module private Writers =
        ...
        type AbstractWriter<'a>() =
            ...
            interface IWriter with
                ...
                member x.WriteUnsafeValue(value, ptr) =
                    ...
                    | _ -> raise <| ArgumentException($"Cannot write {value} (expected: {typeof<'a>})", nameof value)
        ...
        type Type with
            member x.IsBlittable =
                blitCache.GetOrAdd(x, fun x -> 
                    ...
                    let p = t.GetProperty("IsBlittable", BindingFlags.Static ||| BindingFlags.Public ||| BindingFlags.NonPublic)
                    p.GetValue null |> unbox<bool>
                )
        ...
        let private newPrimitiveWriter (t : Type) =
            ...
            let prop = tWriter.GetProperty("Instance", BindingFlags.Static ||| BindingFlags.Public ||| BindingFlags.NonPublic)
        ...
        let private newMemberWriter (mi : MemberInfo) (inner : IWriter) =
            ...
            | _ -> raise <| ArgumentException($"Invalid member info: {mi}.")
        ...
        type MemberInfo with
            member this.Type =
                ...
                | _ -> raise <| ArgumentException($"Invalid member info: {this}.")
        ...
        let rec tryCreateWriterInternal (target : FShade.GLSL.GLSLType) (tSource : Type) =
            ...
            | Struct(_, fields, size) ->
                let fieldWriters =
                    if FSharpType.IsUnion(tSource, true) then
                        ...
                        fields |> List.mapOptionV (fun (name, typ, offset) ->
                            match Map.tryFindV name table with
                            | ValueSome pi ->
                        ...
                        fields |> List.mapOptionV (fun (name, typ, offset) ->
                            ...
                            if isNull fi then
                                ...
                                if isNull pi then
                                    ...
                                    | ValueSome inner ->
                                        ValueSome (nativeint offset, newPropertyWriter pi inner)
                                    | _ ->
                                        ValueNone
                                ...
                                | ValueSome inner ->
                                    ValueSome (nativeint offset, newFieldWriter fi inner)
                                | _ ->
                                    ValueNone
                        )
                match fieldWriters with
                | ValueSome fieldWriters ->
                    ValueSome (newStructWriter tSource (nativeint size) fieldWriters)
                | _ ->
                    ValueNone

            | Array(len, itemType, stride) ->
                let stride = nativeint stride
                match tSource with
                | TypeMeta.Patterns.ArrayOf tSourceElem ->
                    match tryCreateWriterInternal itemType tSourceElem with
                    | ValueSome elemWriter ->
                        if elemWriter.IsPrimitive then
                            if elemWriter.TargetSize = stride then
                                newPrimitiveArrayWriter tSourceElem len |> ValueSome
                            else
                                // TODO: can be improved?
                                newArrayWriter tSourceElem len stride elemWriter |> ValueSome
                        else
                            newArrayWriter tSourceElem len stride elemWriter |> ValueSome
                    | _ ->
                        ValueNone
                ...
                | ArrOf(tLength, tSourceElem) ->
                    match tryCreateWriterInternal itemType tSourceElem with
                    | ValueSome elemWriter ->
                        newArrWriter tLength tSourceElem len stride elemWriter |> ValueSome
                    | _ ->
                        ValueNone

                | TypeMeta.Patterns.SeqOf tSourceElem ->
                    match tryCreateWriterInternal itemType tSourceElem with
                    | ValueSome elemWriter ->
                        newSeqWriter tSource len stride elemWriter |> ValueSome
                    | _ ->
                        ValueNone
                ...
                | _ ->
                    ...
                    | ValueSome elemWriter ->
                        newReplicateWriter elemWriter stride len |> ValueSome
                    | _ ->
                        ValueNone
            ...
            | t ->
                ...
                if tTarget = tSource then
                    ...
                    newMapWriter tSource tTarget converter prim |> ValueSome

    let cache = System.Collections.Concurrent.ConcurrentDictionary<struct (FShade.GLSL.GLSLType * Type), IWriter voption>()

    let tryGetWriter (offset : int) (tTarget : FShade.GLSL.GLSLType) (tSource : Type) =
        try
            let key = struct (tTarget, tSource)
            let writer = cache.GetOrAdd(key, fun struct (tTarget, tSource) -> Writers.tryCreateWriterInternal tTarget tSource)

            match writer with
            | ValueSome w -> w.WithOffset (nativeint offset) |> Result.Ok
            | _ ->
                Result.Error $"Unknown conversion from {tSource} to {GLSLType.toString tTarget}."
        with
