    ...
    type Context with
        member x.CreateUniformBuffer(dataSize : nativeint) =
            Operators.using x.ResourceLock (fun _ ->
                ...
                GLExt.NamedBufferData(handle, dataSize, 0n, BufferUsageHint.DynamicDraw)
        ...
        member x.CreateUniformBuffer(block : ShaderBlock) =
            Operators.using x.ResourceLock (fun _ ->
                ...
                GLExt.NamedBufferData(handle, nativeint block.DataSize, 0n, BufferUsageHint.DynamicDraw)
                GL.Check "could not allocate uniform buffer"
        ...
        member x.Upload(b : UniformBuffer) =
            if b.Dirty then
                b.Dirty <- false
                Operators.using x.ResourceLock (fun _ ->
                    GLExt.NamedBufferSubData(b.Handle, 0n, nativeint b.Size, b.Data)
