        ...
        member x.Commit() =
            lock x (fun () ->
                ...
                if refCount = 1 then
                    ...
                    wait()
                    GL.Dispatch.NamedBufferPageCommitment(buffer, offset, size, true)
                    fence <-Fence.Create()
                else
                    wait()
            )
        ...
        member x.Decommit() =
            lock x (fun () ->
                ...
                if refCount = 0 then
                    Interlocked.Add(totalSize, int64 -size) |> ignore
                    wait()
                    GL.Dispatch.NamedBufferPageCommitment(buffer, offset, size, false)
                    fence <- Fence.Create()
                else
                    wait()
            )
            ...
            if c <> x.SizeInBytes then
                lock x (fun () ->
                    ...
                    if copySize > 0n then
                        ...
                        GL.Dispatch.NamedBufferData(handle, c, 0n, BufferUsageHint.StaticDraw)
...
module SparseBufferExtensions =
    ...
    module private SparseHelpers =
        let rec tryAlloc (cap : byref<nativeint>) =
            ...
            GL.Dispatch.NamedBufferStorage(b, cap, 0n, BufferStorageFlags.SparseStorageBit ||| BufferStorageFlags.DynamicStorageBit)
            let err = GL.GetError()
