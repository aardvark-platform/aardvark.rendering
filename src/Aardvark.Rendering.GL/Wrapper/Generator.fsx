        ...
        if v.StartsWith "(" && v.EndsWith ")" then
            ...
            if m.Success then
                ...
                if v.Contains "." then
                    ...
                    System.Convert.ToInt64(v, b) |> int
            else
...
module FSharpWriter =
    ...
    let reservedKeywords = Set.ofList ["module"; "type"; "object"; "SFRRectCount"; "end"; "class"; "type"; "params"; "val"; "base"; "in" ]
    ...
    let commands (l : list<Command>) =
        printfn "module GLRaw = "

        printfn "    [<Literal>]"
        printfn "    let lib = \"opengl\""
        printfn ""
        for c in l do
    ...
    let extRx = System.Text.RegularExpressions.Regex @"^GL_(?<kind>[A-Z]+)_(?<name>.*)$"
...
//    let s = response.GetResponseStream()
    let path = Path.Combine(__SOURCE_DIRECTORY__, "gl.xml")
    ...
    let file = Path.Combine(__SOURCE_DIRECTORY__, "GL.fs")
