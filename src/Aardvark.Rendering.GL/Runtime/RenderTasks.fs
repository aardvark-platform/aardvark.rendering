...
//                last <- Some s
    ...
    type NativeRenderProgram(cmp : IComparer<PreparedCommand>, scope : CompilerInfo, content : aset<PreparedCommand>) =
        inherit NativeProgram<PreparedCommand>(ASet.sortWith (curry cmp.Compare) content, fun l r s -> r.Compile(scope, s, l)) // s.Compile(scope,Option.map snd l,r))
                ...
                let newProgram = 
                    match config.execution, config.redundancy with
                        ...
                        | ExecutionEngine.Native, RedundancyRemoval.Static -> 
                            ...
                            new NativeRenderProgram(comparer, scope, objects) :> IRenderProgram
