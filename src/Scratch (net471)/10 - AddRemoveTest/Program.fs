...
let main argv = 
    ...
    let should = Mod.init false
    let should2 = Mod.init false

    let gouh = 
        should |> Mod.map (fun should -> 
            if should then 
               quadSg
    ...
    let sg = should2 |> Mod.map (fun should -> if should then sg else Sg.empty) |> Sg.dynamic
