...
module HarwareFeatures =
    ...
    let newWin() =
        let app =
            match useVulkan with
                | true -> new VulkanApplication() :> IApplication
                | false -> new OpenGlApplication() :> IApplication
        let win = 
            if game && not useVulkan then (unbox<OpenGlApplication> app).CreateGameWindow(samples) :> IRenderWindow
    ...
    let instancing() =
        ...
        show {
            ...
            backend (if useVulkan then Backend.Vulkan else Backend.GL)
            samples 8
            debug false
            scene sg
        }
    ...
    let indirect() =
        ...
        let b = if useVulkan then Backend.Vulkan else Backend.GL
        show {
            display Display.Mono
            backend b
            samples 1
