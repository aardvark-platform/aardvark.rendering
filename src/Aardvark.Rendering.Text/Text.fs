...
module ShapeList =
    ...
    // Keep unpaired surrogates literal: they cannot match half of a supplementary glyph.
    // Unlike Rune, these APIs are available on netstandard2.0.
    let private replacementCodePoint (str : string) (i : int) =
        let c = str.[i]
        if Char.IsHighSurrogate c && i + 1 < str.Length && Char.IsLowSurrogate str.[i + 1] then
            struct (Char.ConvertToUtf32(c, str.[i + 1]), i + 2)
        else
            struct (int c, i + 1)

    let private replaceStringMany (str : string) first next (rep : Box2d -> list<ConcreteShape>) shapes =
        // Decode once. A single code point needs no pattern array; unused UTF-16 tail capacity
        // avoids another allocation/copy when the pattern contains surrogate pairs.
        let pattern = if next = str.Length then Array.empty<int> else Array.zeroCreate<int> (str.Length - next)
        let mutable offset = next
        let mutable count = 0
        while offset < str.Length do
            let struct (value, next) = replacementCodePoint str offset
            pattern.[count] <- value
            count <- count + 1
            offset <- next

        // Emit each output node once. Using the collector directly avoids both a reversed
        // output copy and the sequence iterators generated by nested list comprehensions.
        let mutable output = Microsoft.FSharp.Core.CompilerServices.ListCollector<ConcreteShape>()
        let mutable remaining = shapes
        let mutable suffix = []
        while not (List.isEmpty remaining) do
            let head = remaining.Head
            let tail = remaining.Tail
            match head.shape with
            | :? Glyph as g when g.CodePoint.Value = first ->
                let mutable rest = tail
                let mutable i = 0
                let mutable matched = true
                while matched && i < count do
                    match rest with
                    | h :: t ->
                        match h.shape with
                        | :? Glyph as g when g.CodePoint.Value = pattern.[i] ->
                            rest <- t
                            i <- i + 1
                        | _ -> matched <- false
                    | [] -> matched <- false
                ...
                if matched then
                    // Only complete matches need bounds. Failed shared prefixes should not repeatedly
                    // transform glyph bounds that will never be passed to the callback.
                    let mutable bounds = Box.Union(Box2d.Invalid, head.bounds)
                    let mutable matchedShapes = tail
                    for _ in 1 .. count do
                        bounds <- Box.Union(bounds, matchedShapes.Head.bounds)
                        matchedShapes <- matchedShapes.Tail
                    let mutable replacement = rep bounds
                    if List.isEmpty rest then
                        // As with list append, the final replacement can be shared rather than copied.
                        suffix <- replacement
                    else
                        while not (List.isEmpty replacement) do
                            output.Add replacement.Head
            ...
            | _ ->
                output.Add head
                remaining <- tail
        output.AddManyAndClose suffix

    let private replaceStringNonempty (str : string) rep shapes =
        match shapes with
        | [head] ->
            match head.shape with
            | :? Glyph as g ->
                // A longer pattern cannot match a singleton; do not allocate/decode its tail.
                let struct (first, next) = replacementCodePoint str 0
                if next = str.Length && g.CodePoint.Value = first then
                    rep (Box.Union(Box2d.Invalid, head.bounds))
                else
                    [head]
            | _ -> [head]
        | shapes ->
            let struct (first, next) = replacementCodePoint str 0
            replaceStringMany str first next rep shapes

    /// Replaces ordinal, non-overlapping glyph-code-point matches from left to right, without
    /// Unicode normalization. Non-glyph shapes interrupt matches; failed prefixes advance one shape.
    /// Calls rep once per match with the union of the matched shapes' transformed local bounds
    /// (without renderTrafo). Replacement shapes retain their order and are not searched again.
    /// Preserves all other ShapeList fields and does not modify the input. Both shape and pattern
    /// traversal are stack-bounded. Unpaired surrogates match only their literal code-point values.
    /// Throws ArgumentException for an empty pattern before invoking rep; callback exceptions propagate.
    let replaceString (str : string) (rep : Box2d -> list<ConcreteShape>) (s : ShapeList) =
        if str.Length = 0 then invalidArg "str" "The search pattern must not be empty."
        let shapes =
            if List.isEmpty s.concreteShapes then []
            else replaceStringNonempty str rep s.concreteShapes
        { s with concreteShapes = shapes }
